...
class Ship:
    ...
    def angle_to_vector(self):
        self.forward_vector = [math.cos(self.angle), math.sin(self.angle)]
    ...
    def update(self):
        ...
        if self.thrust:
            ...
            self.vel[0] += acceleration[0] * ACCELERATION_RATE
            self.vel[1] += acceleration[1] * ACCELERATION_RATE
    ...
    def shoot(self):
        ...
        missile_vel = [self.vel[0] + 6 * forward_vector[0], self.vel[1] + 6 * forward_vector[1]]
...
def draw(canvas):
    ...
    canvas.draw_image(debris_image, center, size, (wtime + WIDTH / 2, HEIGHT / 2), (WIDTH, HEIGHT))     
    canvas.draw_text('Lives: ' + str(lives), (WIDTH / 15 , HEIGHT / 15), 35, 'Red')
    canvas.draw_text('Score: ' + str(score), (WIDTH / 1.3, HEIGHT / 15), 35, 'White') 
...
def rock_spawner():
    ...
    rock_vel = [random.random() * 1 - .25, random.random() * 1 - .25]
